  ...
  //직진 신호 :: 2,3,6,7
    ...
    if (pp_.mode == 9) {
        ...
        else if(parking_space_flag==true)
        {
          ...
          if(step==1)
          {
            if(result_time<5.5) //endcoder_moved_distance를 받아오는 방법과 target_distance 받아오는 방법만 구현하면 끝 원래 5.45초 -> 5.55
            {
          ...
          // 좌측 후진 원래 6.5->6.6->6.65
          if (result_time < 6.7&& step == 3) {
               std::cout<<result_time<<std::endl;
          ...
          else if (6.7 <= result_time && step == 3) {
               std::cout<<result_time<<std::endl;
